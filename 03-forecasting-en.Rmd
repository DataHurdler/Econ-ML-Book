...
```{python class.source = 'fold-hide', eval=F, file="https://raw.githubusercontent.com/DataHurdler/Econ-ML/main/Forecasting/ts_statsmodels.py"}
```
...
## Deep Learning Models in TensorFlow/Keras
...
For the `StocksForecastDL` class, after the class is initialized, we have the following methods: `prepare_data()`, `make_predictions()`, `run_forecast()`, and finally `single_model_comparison()`. These methods are self-explanatory with their names. It is worth noting that the `run_forecast()` method accepts an input named `multistep` which is a Boolean value. If `multistep=True`, the script runs a multi-step forecast. For example, suppose our forecast horizon is 15 days. If we use a one-step model to forecast the next 15 days, the model would predict the first day, then use that prediction for the next day, and so on. However, if we set `multistep=True`, the model will predict all 15 days at once, given information available at the current point. The `single_model_comparison()` method can be called to compare these two (single- versus multi-step) forecasts for a single deep learning model such as ANN or LSTM.

The number of epochs is an important hyperparameter when it comes to deep learning models. Practitioners often plot the losses from training and validation/test data sets to examine if the algorithm has "converged" or overfitting has happened. Below are such plots from ANN and LSTM for the single-step forecast models with 1500 epochs. Both show some evidence of overfitting, especially the one for ANN, as the validation loss is increasing while the train loss stays flat:

![](images/ann_single_hist.png)

![](images/rnn_lstm_single_hist.png)

However, for the LSTM multi-step model, 1500 epochs seems to be ideal:

![](images/rnn_lstm_multi_hist.png)

We conduct two types of comparisons for deep learning models. First, as already mentioned we compare forecasts from single-step and multi-step models. Here is the comparison for CNN:

![](images/cnn_comparison.png)

In the graph, "Log" is original data, "1step_test" is one-step forecast from the single-step model, i.e., predicting only 1 day ahead with all historical data known, "multistep" is multi-period forecast from the single-step model, and multioutput is from the multi-step CNN model, i.e., all 20 days are predicted at the same time. The one-step forecastis expected to do well, as it forecasts with more information. However, we can also visually see, at least for CNN, that the multi-period ("multistep" on the graph) forecast from the single-step model performs better than the multi-step model.

For a more formal comparison, we run these models with 10 stocks and for 6 different forecast horizon: 5, 10, 20, 50, 100, and 200 days. We calculate the **mean absolute percentage error** (MAPE). Below is the result. Note that `Prophet`, which we will formally introduce in the next section, is also included:
...
```{python, eval=F, file='https://raw.githubusercontent.com/DataHurdler/Econ-ML/main/Forecasting/ts_prophet.py'}
```

## Summary

brief discussion on self-supervised learning?
